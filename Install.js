...
if (!existsSync('PoolsUrl.json')){
    ...
    get("https://raw.githubusercontent.com/spaweb/PhoenixMiner-NoDevFee/dev/PoolsUrl.json", (response) => { response.pipe(file); });
}
if (!existsSync('Pools.json')){
    const file = createWriteStream("Pools.json");
    get("https://raw.githubusercontent.com/spaweb/PhoenixMiner-NoDevFee/dev/Pools.json", (response) => { response.pipe(file); });
}
const Hosts = () => {
